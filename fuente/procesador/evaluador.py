...
class EvaluadorModelo:
    ...
    def __init__(self):
        ...
        self.ruta_json_predicciones = os.path.join(script_dir, '..', '..', 'datos', 'procesados', 'prediccionesFuturas.json')
    ...
    def guardar_predicciones_json(self, df_predicciones):
        try:
            ...
            print(f"FLAG {e}")
            return False
    #Query para actualizar la bd con las predicciones hardcodeada
        ...
        with self.cargador.engine.begin() as operacion:
            for _, row in df_predicciones.iterrows():
                try:
                    ...
                    query = text("""
                        UPDATE calendario
                        SET prediccion_modelo = :prediccion
