...
def cargar_datos(ruta_json):
    ...
    # Separar features y target 32 bits para compatibilidad con tpyorch 'resultado_ht', 'goles_local_final', 'goles_visitante_final'
    X = df.drop(['resultado_final'], axis=1).values.astype('float32')
...
def entrenar_red(ruta_json, cantidad_epocas, batch_size, learning_rate):
    ...
    # Obtener directorio del script para guardar el modelo
    ...
    for epoca in range(cantidad_epocas):
        ...
        if val_loss < mejor_val_loss:
            ...
            ruta_modelo = os.path.join(script_dir, '..', '..', 'datos', 'procesados', 'modelo.pth')
...
def main():
    ...
    # Datos preprocesados
    directorioPreprocesados= os.path.join(directorio, 'procesados', 'partidosPreprocesados.json')

    print("Iniciando entrenamiento de la red neuronal...")
    print("Archivo de datos:", directorioPreprocesados)
    ...
    modelo, val_dl = entrenar_red(
        ruta_json=directorioPreprocesados,
