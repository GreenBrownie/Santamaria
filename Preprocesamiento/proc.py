...
from conexion import CargadorDatos
from config import COLUMNAS_NUMERICAS, MAPEO_RESULTADO, COLUMNAS_USAR
...
class ProcesadorDatos:
    ...
    def cargar_datos(self, limite: int = None) -> pd.DataFrame:
        if limite:
            df = self.cargador.cargar_muestra(n=limite)
        else:
    ...
    def procesar(self, limite: int = None) -> pd.DataFrame:
        """
        Procesamiento:
        1.Cargar datos
        ...
        df = self.cargar_datos(limite=limite)
    ...
    def guardar_json(self, orient: str = 'records') -> str:
        ...
        ruta_completa = os.path.join(script_dir, 'datos_procesados.json')
        
        # Guardar el DataFrame como JSON
        self.df_procesado.to_json(ruta_completa, orient=orient, date_format='iso', indent=2)
